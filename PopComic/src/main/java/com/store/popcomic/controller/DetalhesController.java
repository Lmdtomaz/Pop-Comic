            ...
            //    @GetMapping("/produto/detalhes/{id}")
            //    public ModelAndView visualizarProdutoDetalhes(@PathVariable("id") Long id) {         Estava dando erro !Emanuel!
            ...
            //    public ModelAndView adicionarAoCarrinho(@PathVariable("id") Long id, @RequestParam("redirect") String redirect) { Estava dando erro !Emanuel!
            //        Optional<Produto> produtoOpt = produtoRepository.findById(id);
            //
            //        if (produtoOpt.isPresent()) {
            //            Produto produto = produtoOpt.get();
            ...
            //    }
                @PostMapping("/produto/comprar/{id}")
                public ModelAndView adicionarAoCarrinho(@PathVariable("id") Long id) {
                    Optional<Produto> produtoOpt = produtoRepository.findById(id);

                    if (produtoOpt.isPresent()) {
                        Produto produto = produtoOpt.get();
                        // Aqui você pode realizar qualquer ação necessária ao adicionar o produto
                        // Por exemplo, logar o evento ou realizar outra operação
                    }
